...
// to be processed by the Switch Mediation Engines. The Token process will make sure that only one 
// instance of this will be up and running at a particular time. ie. it prevents the multiple deamons
// of this, running. Additionally, CDR file info is logged to the database so these can be processed by 
